...
//function to input pokemon
var enterpokemon = function()
{
  var pokemonName = prompt("hello there :-\)\), please enter name of a pokemon?");
  while(checkvalid(pokemonName) === false) 
  {
     pokemonName = prompt("please enter a valid pokemon!");
  }
  return pokemonName;

}
 //function to check wether the entered pokemon is valid
 var checkvalid = function(name)
 {
  for(each in pokemonGoData.pokemon)
  {
      if(pokemonGoData.pokemon[each].name.toUpperCase() === name.toUpperCase())
      {
        return true;
      }
  }
...
// q1:function to display the information of a pokemon
var infoOfPokemon = function(key)
{
  console.log("following is the information of " + key);
  for(eachElement in pokemonGoData.pokemon)
  {
    if(key.toUpperCase() === pokemonGoData.pokemon[eachElement].name.toUpperCase())
    {
        console.log(pokemonGoData.pokemon[eachElement]);
    }
  } 
}
...
//q2:function that takes pokemon's name and finds which all pokemon has same name in their "next_evolution"
var nextEvolution = function(key)
{ 
  var found = false;
  for(var i=0; i<pokemonGoData.pokemon.length; i++)
   { 
    
    for(eachEvolution in pokemonGoData.pokemon[i].next_evolution)
     {  
       if(pokemonGoData.pokemon[i].next_evolution[eachEvolution].name.toUpperCase()  === key.toUpperCase())
        {   
            found = true;
            console.log(pokemonGoData.pokemon[i].name + " has " + key + " in its next_evolution");
        }          
     }
   }
   if(found == false)
   {
    console.log(key +" didnt evolve even once.i.e it doesnt come inside any next_evolution");
   }
}



//q3:function that take a “Weakness” as an input and gives the names of all pokemon who have that Weakness
var weaknessesOfPokemon = function(week)
{ 
  var found = false;
  for(var i=0; i<pokemonGoData.pokemon.length; i++)
   { 
    
    for(each in pokemonGoData.pokemon[i].weaknesses)
     {
      if(pokemonGoData.pokemon[i].weaknesses[each].toUpperCase() === week.toUpperCase())
        {   
            found = true;
            console.log(pokemonGoData.pokemon[i].name + " has weaknesses to " + week);
        }          
     }
   }
   if(found== false)
   {
    console.log("no pokemon has the weaknesses of " + week);
   }
}
//execution begins here 
while(true)
{
  var choice = prompt("welcome,choose what you want to know \n" //user enters the number associated with the task
                      + "1: info of a pokemon \n"
                      + "2: nextEvolution \n"
                      + "3: weaknesses ")
  if(choice == 1)
  {
      var key = enterpokemon(); 
      infoOfPokemon(key);
      break;
  }
  else if(choice == 2)
  ...
  {
    var week = prompt("enter a weaknesses")
    weaknessesOfPokemon(week);
    break;
  }
  else
  {
